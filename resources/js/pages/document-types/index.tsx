...
export default function DocumentTypeIndex({ data, filters }: IndexProps) {
    ...
    const handleSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            const queryString = searchData.search ? { search: searchData.search } : undefined;
    ...
    return (
        <>
            ...
            <Heading title="Document Types" description="Manage document type categories." />
                    ...
                    <TableBody>
                        {data.data.length > 0 ? (
                            data.data.map((documentType) => (
                                <TableRow key={documentType.id} className="hover:bg-muted/30 text-sm">
                                    <TableCell className="text-sm font-medium">{documentType.name}</TableCell>
                                    <TableCell className="text-sm">{documentType.description || '-'}</TableCell>
                                    <TableCell className="flex items-center justify-end gap-2 text-sm">
                                        <Button variant="ghost" size="icon" onClick={() => handleEditClick(documentType)}>
                                            <PencilIcon className="h-4 w-4" />
                                        </Button>
                                        <Button variant="ghost" size="icon" onClick={() => handleDeleteClick(documentType)}>
            ...
            {/* Dialogs */}
            {openCreateDialog && (
                <CreateDocumentTypeDialog isOpen={openCreateDialog} onClose={() => setOpenCreateDialog(false)} />
            )}
